...
if ( ! function_exists('getPoolPayout')){
    function getPoolPayout($tournament, $weekNum)
    {
        $ci =& get_instance();

        //load databse library
        $ci->load->database();
        $sql = "SELECT count(DISTINCT userID) AS nrPlayers FROM picks WHERE tournament = '$tournament' AND weekNum ='$weekNum'";
        $stmnt = $ci->db->query($sql);
                    return $stmnt->row_array();
        }
    }
